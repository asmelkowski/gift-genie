...
export default function DrawMetadata({ draw, assignmentCount, groupName }: DrawMetadataProps) {
  ...
  const statusText =
    draw.status === 'finalized'
      ? t('results.information.statusFinalized')
      : t('results.information.statusPending');

  return (
    <div className="bg-card border border-gray-200 rounded-lg p-6">
      {draw.status !== 'finalized' && (
        <div className="mb-4 p-3 bg-yellow-50 border border-yellow-200 rounded-md">
          <p className="text-sm text-yellow-800">{t('results.previewMode')}</p>
        </div>
      )}
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div>
          <h3 className="text-sm font-semibold text-foreground mb-4">
            {t('results.information.title')}
          </h3>
          <div className="space-y-3">
            <div className="flex items-center justify-between">
              <span className="text-gray-600">{t('results.information.statusLabel')}</span>
              <span className={`px-3 py-1 rounded-full text-sm font-semibold ${statusColor}`}>
                {statusText}
              </span>
            </div>
            <div className="flex items-center justify-between">
              <span className="text-gray-600">{t('results.information.createdLabel')}</span>
              <span className="text-foreground">{formatDrawTimestamp(draw.created_at)}</span>
            </div>
            {draw.finalized_at && (
              <div className="flex items-center justify-between">
                <span className="text-gray-600">{t('results.information.finalizedLabel')}</span>
          ...
          <div className="space-y-3">
            {draw.notification_sent_at ? (
              <>
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-green-700 font-medium">
                    {t('results.notification.sentLabel')}
                  </span>
                </div>
              ...
              <div className="flex items-center gap-2">
                ...
                <span className="text-gray-600">{t('results.notification.notSentLabel')}</span>
              </div>
            )}
          </div>
        </div>
