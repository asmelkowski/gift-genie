...
test("should display login page", async ({ page }) => {
  ...
  await expect(page.locator('[data-testid="login-page-title"]')).toContainText(
    "Login",
  );
...
test("should navigate to groups page when logged in", async ({
  authenticatedPage,
}) => {
  await authenticatedPage.goto("/app/groups");
  await expect(
    authenticatedPage.locator('[data-testid="groups-page-header"]'),
