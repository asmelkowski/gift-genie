...
test.describe('Permission Management', () => {
  ...
  test('admin can grant permission to user', async ({ page, context }) => {
    ...
    console.log(
      `[E2E] Initial permission count: ${initialCount || '0'}`
    );
    ...
    const isPermissionGrantedBefore =
      await adminDashboard.isPermissionGranted('draws:notify');
    ...
    const isPermissionGrantedAfter =
      await adminDashboard.isPermissionGranted('draws:notify');
    ...
    const newCountNumber =
      newCount === '' || newCount === '0' ? 0 : parseInt(newCount);
   ...
   test('admin can revoke permission from user', async ({ page, context }) => {
    ...
    console.log('[E2E] Opened permission dialog');
     ...
     const finalCountNumber =
       finalCount === '' || finalCount === '0' ? 0 : parseInt(finalCount);
   ...
   */
  test('permission count updates correctly after grant', async ({
    page,
    context,
  }) => {
    ...
    const initialCountNumber =
      initialCount === '' || initialCount === '0' ? 0 : parseInt(initialCount);
    console.log(
      `[E2E] Initial permission count: ${initialCountNumber}`
    );
   ...
   */
  test('permission count updates correctly after revoke', async ({
    page,
    context,
  }) => {
    ...
    const beforeCountNumber =
      beforeCount === '' || beforeCount === '0' ? 0 : parseInt(beforeCount);
    ...
    const afterCountNumber =
      afterCount === '' || afterCount === '0' ? 0 : parseInt(afterCount);
   ...
   */
  test('permission appears in user list immediately after grant', async ({
    page,
    context,
  }) => {
   ...
   */
  test('permission removed from user list immediately after revoke', async ({
    page,
    context,
  }) => {
   ...
   */
  test('admin can search for user before managing permissions', async ({
    page,
    context,
  }) => {
    ...
    console.log(
      '[E2E] ✓ Permission granted to correct user via search workflow'
    );
  });
    ...
    */
  test('multiple permissions can be managed in sequence', async ({
    page,
    context,
  }) => {
